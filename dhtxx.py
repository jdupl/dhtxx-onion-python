...
class DHT:
    """Reader class for DHTxx for GPIO library"""
    def __init__(self, pin):
        """
        pin: GPIO pin number
        """
        self.pin = pin
        self.gpioObj = onionGpio.OnionGpio(self.pin)
    ...
    def _get_bytes_from_dht(self):
        ...
        try:
            ...
            self.gpioObj.setOutputDirection(1)

            # Pull down for 20ms
            # GPIO.output(self.pin, GPIO.LOW)
            self.gpioObj.setValue(1)
            sleep(20 / 1000.0)

            # GPIO.output(self.pin, GPIO.HIGH)
            self.gpioObj.setValue(0)

            # GPIO.setup(self.pin, GPIO.IN)
            self.gpioObj.setInputDirection()
...
class DHT22(DHT):
    ...
    def _get_temp_humidity_tuple(self, byte_array):
        ...
        return round(temp, 1), round(rel_humidity, 1)
